...
{
    static NetAnalyticData* sington = NULL;
    if(sington == NULL) {
        sington = new NetAnalyticData();
    }
    return sington;
}
 ...
 * 返回值：校验码
 */
uchar NetAnalyticData::net_data_xor(uchar *buf, ushort len)
{
    ushort i;
    uchar XOR=0;
 ...
 */
int NetAnalyticData::data_packet_sync(uchar *buf, ushort len)
{
    int ret=0,rtn = -1;
...
int NetAnalyticData::dev_data_analytic(uchar *buf, ushort len, Net_sDevData *pkt)
{
    ...
    if(rtn < len)
    {
        ...
        qDebug() << "dev_data_analytic err" << rtn << len;
        rtn = 0;
    }
